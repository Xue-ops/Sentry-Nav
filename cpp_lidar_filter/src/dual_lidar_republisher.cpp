...
public:
  ...
  : Node("dual_lidar_republisher")
  {
    ...
    declare_parameter("front_imu_input_topic", "/livox/imu_192_168_1_188");
    declare_parameter("front_imu_output_topic", "/mid360_front/imu");
    ...
    declare_parameter("back_imu_input_topic", "/livox/imu_192_168_1_182");
    declare_parameter("back_imu_output_topic", "/mid360_back/imu");
    ...
    const auto front_imu_input_topic = get_parameter("front_imu_input_topic").as_string();
    const auto front_imu_output_topic = get_parameter("front_imu_output_topic").as_string();
    ...
    const auto back_imu_input_topic = get_parameter("back_imu_input_topic").as_string();
    const auto back_imu_output_topic = get_parameter("back_imu_output_topic").as_string();
    ...
    front_imu_publisher_ = create_publisher<sensor_msgs::msg::Imu>(front_imu_output_topic, qos);
    back_imu_publisher_ = create_publisher<sensor_msgs::msg::Imu>(back_imu_output_topic, qos);
    ...
    front_imu_subscription_ = create_subscription<sensor_msgs::msg::Imu>(
      front_imu_input_topic, qos,
      [this, front_frame_id](sensor_msgs::msg::Imu::ConstSharedPtr message) {
        auto output = *message;
        output.header.frame_id = front_frame_id;
        front_imu_publisher_->publish(output);
      });

    back_imu_subscription_ = create_subscription<sensor_msgs::msg::Imu>(
      back_imu_input_topic, qos,
      [this, back_frame_id](sensor_msgs::msg::Imu::ConstSharedPtr message) {
        auto output = *message;
        output.header.frame_id = back_frame_id;
        back_imu_publisher_->publish(output);
      });
    ...
    RCLCPP_INFO(
      get_logger(), "Front IMU: %s -> %s (frame_id=%s)",
      front_imu_input_topic.c_str(), front_imu_output_topic.c_str(), front_frame_id.c_str());
    RCLCPP_INFO(
      get_logger(), "Back IMU: %s -> %s (frame_id=%s)",
      back_imu_input_topic.c_str(), back_imu_output_topic.c_str(), back_frame_id.c_str());
  }
...
private:
  ...
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr front_imu_publisher_;
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr back_imu_publisher_;
  ...
  rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr front_imu_subscription_;
  rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr back_imu_subscription_;
};
