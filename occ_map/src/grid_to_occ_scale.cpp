...
class GridMapToOccScaleNode : public rclcpp::Node
{
public:
  GridMapToOccScaleNode()
  : Node("gridmap_to_occ_scale_node")
  {
    ...
    max_slope_cost_ = this->declare_parameter<double>("max_slope_cost", 35.0);
    min_slope_cost_ = this->declare_parameter<double>("min_slope_cost", 1.0);
    ...
    grid_sub_ = this->create_subscription<grid_map_msgs::msg::GridMap>(
      input_topic_, 10,
      std::bind(&GridMapToOccScaleNode::gridMapCallback, this, std::placeholders::_1));

    RCLCPP_INFO(this->get_logger(), "gridmap_to_occ_scale_node started");
...
private:
  void gridMapCallback(const grid_map_msgs::msg::GridMap::SharedPtr msg)
  {
    ...
    for (grid_map::GridMapIterator it(grid_map); !it.isPastEnd(); ++it) {
      ...
      if (occupied) {
        occ(index(0), index(1)) = 100.0f;
        } else {
        double ratio = 0.0;

        if (max_obstacle_height_ > min_obstacle_height_) {
            ratio = (static_cast<double>(elevation) - min_obstacle_height_) /
        ...
        const float cost = static_cast<float>(
            min_slope_cost_ +
            ratio * (max_slope_cost_ - min_slope_cost_)
        );

        occ(index(0), index(1)) = cost;
        }
    }
...
private:
  ...
  double max_slope_cost_; 
  double min_slope_cost_;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<GridMapToOccScaleNode>());
  rclcpp::shutdown();
  return 0;
}